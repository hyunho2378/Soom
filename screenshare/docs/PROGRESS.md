...
## BUILD_SPEC v2 D그룹: 모바일, 셀 확대, 아이콘 색, 순차 업로드, 정원 문구 (2026-08-12)

### D2. 모바일 화면 공유 차단
- getDisplayMedia가 없거나, 정밀 포인터가 아예 없는 손가락 전용 기기면 공유 버튼을 잠그고 이유를 적는다.
  판정은 (pointer: coarse)이면서 (any-pointer: fine)이 아닐 때다. 터치 노트북은 정밀 포인터가 함께 있어 통과한다.
  maxTouchPoints는 페이지 이동 시점에 따라 0으로 잡히는 경우가 있어 기준으로 쓰지 않았다.
- 기록물 올리기는 그대로 쓸 수 있다고 문구로 알린다.

### D3. 셀 제자리 확대
- 전체 화면 오버레이를 걷어내고 격자 안에서 확대한다. 확대한 셀이 격자 자리를 차지하고
  나머지는 아래 작은 띠(148px)로 남는다. 셀을 다시 누르거나 ESC로 격자로 돌아온다.
- 셀이 하나뿐이면 이미 가장 크므로 확대를 막고 손잡이도 감춘다.
- 확대 상태는 렌더마다 다시 입혀 셀이 들고 나도 유지된다.

### D4. 파일 종류별 아이콘 색
- PDF 빨강, 워드 파랑, HTML 주황, 텍스트 회색, 마크다운 보라. 아이콘이 currentColor라 색만 바꾼다.
- 보라는 --purple 토큰을 새로 넣었다.

### D5. Blob 순차 업로드
- Promise.all 병렬을 for 루프 순차로 바꾸고 올린 버퍼는 즉시 참조를 끊는다.
  무료 플랜 RAM이 512MB라 큰 파일 여러 개가 한꺼번에 메모리에 남으면 위험하다.

### D6. 정원 초과 문구
- "현재 공유 가능 인원이 꽉 찼습니다. 한 명이 멈추면 다시 시도하세요(최대 10명)."

### 검증 중 함께 고친 것
- 연결 상태 바가 sticky 회의실 헤더를 덮어 방 코드가 가려졌다. 바가 떠 있는 동안 body에 여백을 주고
  헤더의 sticky 기준점을 바 높이만큼 내렸다.

### 검증(C그룹과 함께 17개 항목 통과)
- 데스크탑에서는 공유 버튼이 살아 있고, 모바일 에뮬레이션에서는 비활성 + "PC에서만 가능합니다" 안내.
- 체험자 둘 공유 상태에서 셀을 누르면 확대(격자 폭 100퍼센트), 나머지 1개가 절반 미만 크기로 아래에 남음.
  다시 누르면 복귀, ESC로도 복귀. 옛 오버레이 요소는 DOM에 없음.
- 아이콘 색 5종이 계산값으로 정확히 일치.
...
## UI 수정 2건과 격자 동시 표시 버그

### 1. 체험자도 전체 화면으로 본다
- 강연자 격자에만 있던 전체 화면 토글을 체험자 쪽에도 붙였다.
- viewerFrame을 viewerWrap으로 감싸고 격자와 같은 머리줄(grid-head) 구조를 썼다.
  제목은 "공유 화면"이다. 이 자리는 강연자 시범도, 내가 보내는 화면 미리보기도 띄우기 때문이다.
- 전체 화면 처리는 toggleFullscreen 하나로 합쳐 강연자와 체험자가 같은 길을 쓴다.
- 전체 화면에서는 16대 9 고정을 풀고 남는 세로를 다 쓰게 했다.

### 2. 여러 체험자 화면이 동시에 안 보이던 버그
- 격자 코드에는 문제가 없었다. 셀은 id로 구분해 붙이고 지운다.
- 진짜 원인은 C그룹에서 넣은 stale 정리다. 같은 이름과 같은 역할이면 앞사람을 쫓아냈다.
  동명이인이나 이름을 비운 사람들("익명")이 서로를 내보낸다. 재현으로 확인했다.
  뒤에 온 사람이 앞사람 소켓을 닫는데, removePublisher를 거치지 않아
  강연자에게 publisher-stopped도 안 갔다. 격자에 죽은 셀이 남는다.
- 이름은 신원이 아니다. 판정 근거를 클라이언트 id로 바꿨다. id는 탭마다 고유하다.
- id를 sessionStorage에 담아 새로고침해도 유지시켰다. 죽은 소켓 정리라는 원래 목적은 그대로 지킨다.
- 옛 소켓의 close가 새로 들어온 본인 자리를 지우던 문제도 함께 막았다.
  자리에 앉은 소켓이 자기가 아니면 close 처리를 건너뛴다.
- 공유 중이던 자리(publishers)는 이제 소켓 교체를 넘어 유지된다. 같은 사람이 이어받기 때문이다.

### 3. 확대해서 보는 중에 새 사람이 들어와도 화면이 안 바뀐다
- 이미 그렇게 동작했다. applyZoomState가 확대 대상이 살아 있는 한 그대로 두고,
  새 셀은 아래 띠에 붙기만 한다. 체험자 3명으로 실제로 만들어 확인했다.

### 검증 (브라우저 4탭, 이름 두 경우 각각 22개 항목 통과)
- 강연자 1 + 체험자 3. A 공유 후 시간을 두고 B가 공유해도 A가 남는다(2셀, 둘 다 영상 도착).
- 서로 다른 이름과 완전히 같은 이름("주현호" 둘) 두 경우 모두 통과.
- A를 확대한 상태에서 C가 새로 공유해도 확대는 A 그대로, C는 아래 띠에 148px로 조용히 붙는다.
- A가 멈추면 A 셀만 사라지고 나머지는 유지된다.
- 참가자 목록에 공유중 표시가 셋 다 뜬다.
- 체험자 전체 화면 켜기, 끄기, 라벨 전환, 머리줄과 영상 프레임 좌우 정렬까지 확인.
- 회귀: ICE 검증 21개, 재접속 정리 7개 모두 통과. 재접속 시 공유 자리를 잃지 않는 것도 확인.
- 검증 흔적은 모두 지웠다. 실사용자 1명과 기록물 1건은 그대로다.
